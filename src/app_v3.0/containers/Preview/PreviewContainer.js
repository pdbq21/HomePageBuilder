...
class PreviewContainer extends Component {
                ...
                switch (elementType) {
                    ...
                    case 'image':
                        if (typeof link === "undefined") {
                            element = (<div className="pb-element pb-image-element">
    ...
    renderCol(id) {
        const {childrenIds} = this.props.mapStateWorkArea[id];
        let data = childrenIds.map((childrenId) => {
            const {gridIndex} = this.props.mapStateWorkArea[childrenId];
            return (
                <div className={`col-${gridIndex}`} key={`key-${childrenId}`}>
                    {this.renderElement(childrenId)}
                </div>
            );
        });
    ...
    renderRow(id) {
        ...
        let data = childrenIds.map((childrenId) => {
            ...
            const {paddingTop, paddingRight, paddingBottom, paddingLeft} = styles.padding;
            return (
                <div className="pb-row written-row" key={`key-${childrenId}`}>
                    <div
    ...
    render() {
        ...
        return (
            <div className="pb-preview" style={{'width': '100%'}}>
                {this.renderPreview()}
            </div>
        );
    }
